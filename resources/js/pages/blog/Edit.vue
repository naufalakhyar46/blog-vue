<template>
<form @submit.prevent="handleSubmit" id="form-update-blog" enctype="multipart/form-data">
<br>
...
<div class="col-md-4">
	<div class="box">
		<div class="box-body">
			<div class="form-group">
			<img v-if="previewImage" :src="previewImage" style="width: 100%" class="gambar">
			<br>
...
export default {
    ...
    methods: {
        getAkun(){
            ...
            axios.get('/api/blog/show/' + this.id).then((response) => {
                console.log(response)
                this.form = response.data;
                if(response.data.image){
                    this.previewImage = '/image/blog/' + response.data.image
                }else{
        ...
        handleSubmit() {
            let formData = new FormData($('#form-update-blog')[0])
            formData.append('image',this.image)
            axios.defaults.headers.common['Content-Type'] = 'application/json'
            axios.defaults.headers.common['Authorization'] = 'Bearer ' + localStorage.getItem('jwt')
            axios.post('/api/blog/' + this.id, formData).then((response) => {
